    ...
    return (
        <div className="tarea">
            ...
            <input className={editando ? "visible" : ""} type="text"
                value={tareaTemporal}
                onChange={evento => setTareaTemporal(evento.target.value)} />
            <button className="boton" onClick={async () => {

                if (editando) {
                    if (tareaTemporal.trim() != "" && tareaTemporal.trim() != tarea) {
                        let { resultado, error } = await fetch(`https://back-mongodb.onrender.com/tareas/actualizar/${id}/1`, {
                            method: "PUT",
            ...
            <button className="boton" onClick={() => {
                fetch("https://back-mongodb.onrender.com/tareas/borrar/" + id, {
                    method: "DELETE"
                })
                    .then(respuesta => respuesta.json())
            ...
            <button className={`estado ${estado ? "terminada" : ""}`} onClick={() => {
                ...
                fetch(`https://back-mongodb.onrender.com/tareas/actualizar/${id}/2`, {
                    method: "PUT"
                })
                    .then(respuesta => respuesta.json())
                    .then(({ resultado, error }) => {
